...
export default function Reports() {
  ...
  const fetchStats = async () => {
    // This would ideally be a single endpoint, but we can aggregate here for the mock
    try {
      const [clients, payments, memberships] = await Promise.all([
        ...
        fetch("http://localhost:3000/api/memberships").then((r) => r.json()),
      ]);

      setStats({
        totalClients: clients.length,
        activeMemberships: memberships.filter((m: any) => m.status === "active")
          .length,
